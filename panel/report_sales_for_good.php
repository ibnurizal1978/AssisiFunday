...
header("Content-Disposition: attachment; filename=sales-for-good-report.xls");
?>
<!-- Main Container -->
<main id="main-container">
                ...
                if(@$shop_id == 'all') {
                  ...
                  $s = "SELECT *, date_format(b.payment_at, '%d-%m-%Y') as buy_date, date_format(b.payment_at, ' %H:%i') as buy_time FROM tbl_order_detail a INNER JOIN tbl_order b USING (order_code) INNER JOIN tbl_product USING (product_id) WHERE sell_for_good = 1 AND (date(b.updated_at) BETWEEN '".$date_from."' AND '".$date_to."') AND order_status = 'COMPLETED'";
                }else{
                   $s = "SELECT *, date_format(b.payment_at, '%d-%m-%Y') as buy_date, date_format(b.payment_at, ' %H:%i') as buy_time FROM tbl_order_detail a INNER JOIN tbl_order b USING (order_code) INNER JOIN tbl_product USING (product_id) WHERE sell_for_good = 1 AND (date(b.updated_at) BETWEEN '".$date_from."' AND '".$date_to."') AND order_status = 'COMPLETED' AND a.shop_id = '".$shop_id."'";
                }
                //echo $s;
